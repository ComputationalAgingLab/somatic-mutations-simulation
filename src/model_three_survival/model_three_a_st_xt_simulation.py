...
class Config:
    ...
    def get_params(self):
        ...
        r_se = (conf['r']['CI_high'] - conf['r']['CI_low']) / (2 * 1.96)
...
def run_pipeline_compare_fixed(organ="liver", n_km=2000, n_traj=400, t_max=200_000,
    ...
    print(f"Config: organ={organ}; b_crit={x_crit:.3e}")
    ...
    for dist in ("lognormal",):
        ...
        km_csv = os.path.join(outdir, f"Model_IIIa_liver_St.csv")
        ...
        pd.DataFrame(extra_rows).to_csv(os.path.join(outdir, f"Model_IIIa_liver_extra_stats.csv"), index=False)
        ...
        if traces:
            ...
            Xmat = np.vstack(Xs) if Xs else np.empty((0, len(ts_common)))
            Pmat = np.vstack(Ps) if Ps else np.empty((0, len(ts_common)))

            meanX = Xmat.mean(axis=0) if Xmat.size else np.full_like(ts_common, np.nan)
            stdX = Xmat.std(axis=0) if Xmat.size else np.full_like(ts_common, np.nan)
            ...
            traj_csv_path = os.path.join(outdir, f"Model_IIIa_liver_Xt.csv")
            ...
            for p in traj_percentiles:
                pctX = np.percentile(Xmat, p, axis=0) if Xmat.size else np.full_like(ts_common, np.nan)
            ...
            pct_csv_path = os.path.join(outdir, f"Model_IIIa_liver_Xt_percentiles.csv")
            pct_df.to_csv(pct_csv_path, index=False)
            print(f"Saved trajectories percentile CSV: {pct_csv_path}")

            mapped_pct_X = {50: pct_dict_X[50], 2.5: pct_dict_X[2.5], 97.5: pct_dict_X[97.5]}
            mapped_pct_P = {50: pct_dict_P[50], 2.5: pct_dict_P[2.5], 97.5: pct_dict_P[97.5]}
            save_trajectory_plots(outdir, f"{organ}_{dist}", ts_common, mapped_pct_X, mapped_pct_P)
        else:
    ...
    summary_path = os.path.join(outdir, f"Model_IIIa_{organ}_summary.csv")
